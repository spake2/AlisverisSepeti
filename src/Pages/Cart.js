    ...
    return (
        <Container className="py-4 mt-5">
            <h1 className={`${theme? 'text-light': 'text-light-primary'} my-5 text-center`}>
                {isEmpty? 'Sepetiniz Boş' : 'Sepetinizdeki Ürünler'}
            </h1>
                    ...
                    <tbody>
                        {items.map((item, index)=>{
                            return(
                                <tr key={index}>
                                    <td>
                                        <div style={{ background: 'white', height: '11rem', overflow: 'hidden', display: 'flex',
                                    ...
                                    <td>
                                        ...
                                        <Button onClick={()=> updateItemQuantity(item.id, item.quantity + 1)} className="ms-2">+</Button>
                                        <Button variant="danger" onClick={()=> removeItem(item.id)} className="ms-2">Sepetten Kaldır</Button>
                                    </td>
                                </tr>
                            )
                        })}
